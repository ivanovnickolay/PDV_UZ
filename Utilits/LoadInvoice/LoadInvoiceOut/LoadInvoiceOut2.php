            ...
            while(!$exit)
            {
                $this->createExcelReader ($FileName);
                  $currentTime=microtime(true)-$startTime;
                    $this->out->writeln("Create Excel reader. Time: $currentTime");
                // Загружаем класс валидации данных
                $validInvoice = new ValidInvoiceOut($this->entityManager);
                    $this->out->writeln("Load ValidInvoiceOut ");
                        $this->chunkFilter->setRows($row,range('A','T'), $this->chunkSize);
                             $this->objPHPExcel = $this->phpExcel->load($Path_FileName);
                                 $this->reconnect();
	                                 $currentTime=microtime(true)-$startTime;
                $this->out->writeln("Load file from phpExcel Time: $currentTime");
                        ...
                        // проверяем строку
                            if (($validInvoice->valid($Invoice))) {
                                ...
                                $this->out->writeln(" Persist Invoice. Row $rowStart");
                                 ...
                                 unset($Invoice);
                                $this->out->writeln(" NO persist Invoice. Row $rowStart");
                            }
                    ...
                    //   if ((!empty($num)))
                        if ($rowStart%$this->flushCount==0)
                        {
                            ...
                            try {
	                            ...
	                            $currentTime=microtime(true)-$startTime;
                                $this->out->writeln (" flush Invoice. Row $rowStart Time: $currentTime");
                            } catch (Exception $e) {
